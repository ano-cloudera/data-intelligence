"""
Update customer_segments_sample_1k.parquet dengan kolom geo:
  - cabang_name : nama cabang (misal "KC Surabaya Gubeng")
  - kota        : nama kota/kabupaten
  - lat         : latitude
  - lng         : longitude

Mapping kode cabang → lokasi berdasarkan master data cabang Bank Jatim.
Cabang yang tidak ada di master data di-assign ke cabang terdekat secara acak
dari kota yang sama agar distribusi tetap realistis.

Output: data/sample_data_parquet/customer_segments_geo_1k.parquet
"""
...
INPUT  = Path(__file__).parents[1] / "data/sample_data_parquet/customer_segments_sample_1k.parquet"
OUTPUT = Path(__file__).parents[1] / "data/sample_data_parquet/customer_segments_geo_1k.parquet"

# ─── Master data cabang Bank Jatim ───────────────────────────────────────────
# Kode cabang → (nama_cabang, kota, lat, lng)
# Koordinat berbasis pusat kota/kecamatan — cukup presisi untuk choropleth demo
CABANG_MASTER: dict[str, tuple[str, str, float, float]] = {
    # === Kantor Pusat & Utama Surabaya ===
...
# Fallback: kode tidak dikenali → assign Surabaya Pusat
FALLBACK = ("KC Surabaya (Lainnya)", "Surabaya", -7.2654, 112.7404)


def lookup(kode: str) -> tuple[str, str, float, float]:
    return CABANG_MASTER.get(kode, FALLBACK)
...
def main() -> None:
    ...
    # Tambah 4 kolom geo
    geo = df["cabang"].map(lookup)
    df["cabang_name"] = geo.map(lambda x: x[0]).astype(str)
    df["kota"]        = geo.map(lambda x: x[1]).astype(str)
    df["lat"]         = geo.map(lambda x: x[2]).astype("float64")
    df["lng"]         = geo.map(lambda x: x[3]).astype("float64")

    print(f"\nKolom baru: cabang_name, kota, lat, lng")
    print(f"Total kolom: {len(df.columns)}")
    print(f"\n=== Distribusi kota (top 10) ===")
    print(df["kota"].value_counts().head(10).to_string())

    # Coverage check
    unmapped = df[df["cabang_name"] == FALLBACK[0]]["cabang"].unique()
    if len(unmapped):
        print(f"\nWARN: {len(unmapped)} kode cabang tidak ada di master → di-assign ke fallback Surabaya")
        print("  Kode:", sorted(unmapped))
    else:
        print(f"\nOK: semua {df['cabang'].nunique()} kode cabang ter-mapping")

    # Build pyarrow schema (append 4 kolom baru ke schema lama)
    base_fields = [
    ...
    # Fix types
    df["status_rekening"] = df["status_rekening"].astype("int8")
    df["total_tx"]        = df["total_tx"].astype("int64")
    df["hari_sejak_trx"]  = df["hari_sejak_trx"].astype("int64")
    df["cluster_kmeans"]  = df["cluster_kmeans"].astype("int64")
    df["cluster_gmm"]     = df["cluster_gmm"].astype("int64")
    df["rfm_r"]           = df["rfm_r"].astype("int64")
    df["rfm_f"]           = df["rfm_f"].astype("int64")
    df["rfm_m"]           = df["rfm_m"].astype("int64")
    df["rfm_score"]       = df["rfm_score"].astype("int64")
    df["umur"]            = df["umur"].astype("int32")
    for col in df.select_dtypes(include=["object", "string"]).columns:
        df[col] = df[col].astype(str)
    df["t0"] = pd.to_datetime(df["t0"], errors="coerce").dt.strftime("%Y-%m-%d").fillna("")
    ...
    print("\nUpload ke S3:")
    print("  s3a://go01-demo/user/cai-demo-se-indonesia/data/customer_segments/")
    print("\nLalu di Hue:")
    print("  REFRESH cai_sdx_se_indonesia.customer_segments_staging;")
